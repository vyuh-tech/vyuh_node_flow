...
class TestPlugin extends NodeFlowPlugin {
  @override
  final String id;

  TestPlugin({this.id = 'test-plugin'});
...
void main() {
  group('NodeFlowPlugin', () {
    ...
    group('Plugin Lifecycle', () {
      test('addPlugin attaches extension', () {
        final extension = TestPlugin();
        expect(extension.attached, isFalse);

        controller.addPlugin(extension);

        expect(extension.attached, isTrue);
        expect(extension.controller, isNotNull);
      });

      test('removePlugin detaches extension', () {
        final extension = TestPlugin();
        controller.addPlugin(extension);
        expect(extension.detached, isFalse);

        controller.removePlugin('test-plugin');

        expect(extension.detached, isTrue);
        expect(controller.hasPlugin('test-plugin'), isFalse);
      });
      ...
      test('dispose detaches all extensions', () {
        ...
        final extension1 = TestPlugin(id: 'ext-1');
        final extension2 = TestPlugin(id: 'ext-2');

        testController.addPlugin(extension1);
        testController.addPlugin(extension2);
      ...
      test('hasPlugin returns true for registered extensions', () {
        final extension = TestPlugin();
        expect(controller.hasPlugin('test-plugin'), isFalse);

        controller.addPlugin(extension);

        expect(controller.hasPlugin('test-plugin'), isTrue);
      });

      test('getPlugin returns typed extension', () {
        final extension = TestPlugin();
        controller.addPlugin(extension);

        final retrieved = controller.getPlugin<TestPlugin>();
      ...
      test('extensions getter returns unmodifiable list', () {
        ...
        final extensions = controller.plugins;
    ...
    group('Multiple Plugins', () {
      test('events are broadcast to all extensions', () {
        ...
        controller.addPlugin(extension1);
        controller.addPlugin(extension2);
