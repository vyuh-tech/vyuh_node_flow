...
///
/// Tests cover:
/// - NodeFlowPlugin base class interface and lifecycle
/// - PluginRegistry for managing extensions
/// - AutoPanPlugin configuration and state
/// - DebugPlugin modes and visual overlay properties
/// - LodPlugin (Level of Detail) thresholds and visibility
/// - MinimapPlugin state and highlighting
/// - StatsPlugin statistics tracking
@Tags(['unit'])
library;
...
void main() {
  ...
  // ===========================================================================
  // NodeFlowPlugin - Interface and Lifecycle
  // ===========================================================================

  group('NodeFlowPlugin - Interface', () {
    test('extension has required id property', () {
      final ext = AutoPanPlugin();
    ...
    test('different extensions have different ids', () {
      ...
      final lod = LodPlugin();
      final minimap = MinimapPlugin();
      final stats = StatsPlugin();
    ...
    test('extension ids follow naming convention', () {
      expect(AutoPanPlugin().id, equals('auto-pan'));
      expect(DebugPlugin().id, equals('debug'));
      expect(LodPlugin().id, equals('lod'));
      expect(MinimapPlugin().id, equals('minimap'));
      expect(StatsPlugin().id, equals('stats'));
    });
  });

  group('NodeFlowPlugin - Lifecycle', () {
    test('extensions are lazily attached when accessed', () {
      final controller = NodeFlowController<String, dynamic>();

      // Plugins are lazily attached when accessed via resolvePlugin
      // Before access, hasPlugin returns false
    ...
    test('extension can be added after controller creation', () {
      ...
      expect(controller.hasPlugin('minimap'), isFalse);

      controller.addPlugin(MinimapPlugin(visible: true));

      expect(controller.hasPlugin('minimap'), isTrue);
  ...
  group('PluginRegistry - Adding Plugins', () {
    ...
    test('creates registry with initial extensions', () {
      final extensions = [AutoPanPlugin(), DebugPlugin()];
      final registry = PluginRegistry(extensions);
    ...
    test('register adds extension to registry', () {
      ...
      registry.register(AutoPanPlugin());
    ...
    test('register replaces existing extension with same id', () {
      final registry = PluginRegistry();
      final original = AutoPanPlugin(panAmount: 10.0);
      final replacement = AutoPanPlugin(panAmount: 20.0);

      registry.register(original);
      registry.register(replacement);

      final retrieved = registry.get<AutoPanPlugin>();
      expect(retrieved?.panAmount, equals(20.0));
    });
  });

  group('PluginRegistry - Removing Plugins', () {
    test('remove deletes extension by id', () {
      final registry = PluginRegistry([AutoPanPlugin(), DebugPlugin()]);
    ...
    test('remove non-existent extension does nothing', () {
      final registry = PluginRegistry([AutoPanPlugin()]);
  ...
  group('PluginRegistry - Getting Plugins', () {
    test('get returns extension by type', () {
      final registry = PluginRegistry([AutoPanPlugin(), DebugPlugin()]);

      final autoPan = registry.get<AutoPanPlugin>();
      final debug = registry.get<DebugPlugin>();

      expect(autoPan, isA<AutoPanPlugin>());
      expect(debug, isA<DebugPlugin>());
    });

    test('get returns null for unregistered type', () {
      final registry = PluginRegistry([AutoPanPlugin()]);

      final result = registry.get<MinimapPlugin>();
    ...
    test('getById returns extension by id', () {
      ...
      expect(result, isA<AutoPanPlugin>());
    });
  ...
  group('PluginRegistry - Has Plugin Check', () {
    ...
    test('all returns all registered extensions', () {
      ...
      expect(registry.all, hasLength(2));
      expect(registry.all.whereType<AutoPanPlugin>(), hasLength(1));
      expect(registry.all.whereType<DebugPlugin>(), hasLength(1));
    });
  });

  // ===========================================================================
  // AutoPanPlugin - Configuration
  // ===========================================================================

  group('AutoPanPlugin - Construction', () {
    ...
    test('creates with disabled state', () {
      final ext = AutoPanPlugin(enabled: false);

      expect(ext.isEnabled, isFalse);
    });
  });

  group('AutoPanPlugin - Configuration Properties', () {
  ...
  group('AutoPanPlugin - Presets', () {
    test('useNormal applies balanced settings', () {
      final ext = AutoPanPlugin(panAmount: 100.0);
  ...
  group('AutoPanPlugin - Enable/Disable', () {
    ...
    test('disable disables autopan', () {
      final ext = AutoPanPlugin(enabled: true);
  ...
  // ===========================================================================
  // DebugPlugin - Construction and Settings
  // ===========================================================================

  group('DebugPlugin - Construction', () {
    test('creates with default values', () {
    ...
    test('creates with custom theme', () {
      final ext = DebugPlugin(theme: DebugTheme.dark);

      expect(ext.theme, equals(DebugTheme.dark));
    });
  });

  group('DebugPlugin - Mode Settings', () {
  ...
  group('DebugPlugin - Visual Overlay Properties', () {
  ...
  // ===========================================================================
  // LodPlugin - Level of Detail
  // ===========================================================================

  group('LodPlugin - Construction', () {
    test('creates with default values', () {
      final ext = LodPlugin();
    ...
    test('creates with enabled state', () {
      final ext = LodPlugin(enabled: true);

      expect(ext.isEnabled, isTrue);
    });

    test('creates with custom thresholds', () {
      final ext = LodPlugin(minThreshold: 0.2, midThreshold: 0.6);
    ...
    test('creates with custom visibility presets', () {
      ...
      final ext = LodPlugin(minVisibility: customVisibility);
    ...
    test('returns correct visibility for zoom zones when enabled', () {
      final controller = NodeFlowController<String, dynamic>(
        config: NodeFlowConfig(
          ...
          maxZoom: 1.0,
          plugins: [
            LodPlugin(enabled: true, minThreshold: 0.25, midThreshold: 0.6),
  ...
  // ===========================================================================
  // MinimapPlugin - Construction and State
  // ===========================================================================

  group('MinimapPlugin - Construction', () {
    test('creates with default values', () {
      final ext = MinimapPlugin();
    ...
    test('creates with visibility enabled', () {
      final ext = MinimapPlugin(visible: true);

      expect(ext.isVisible, isTrue);
    });

    test('creates with custom position', () {
      final ext = MinimapPlugin(position: MinimapPosition.topLeft);

      expect(ext.position, equals(MinimapPosition.topLeft));
    });

    test('creates with custom size', () {
      final ext = MinimapPlugin(size: const Size(300, 200));

      expect(ext.size, equals(const Size(300, 200)));
    });

    test('creates with custom theme', () {
      final ext = MinimapPlugin(theme: MinimapTheme.dark);
  ...
  group('MinimapPlugin - Visibility', () {
    test('show makes minimap visible', () {
      final ext = MinimapPlugin(visible: false);
  ...
  group('MinimapPlugin - Size', () {
    ...
    test('setWidth updates only width', () {
      final ext = MinimapPlugin(size: const Size(200, 150));
  ...
  group('MinimapPlugin - Interactivity', () {
    test('enableInteraction enables interaction', () {
      final ext = MinimapPlugin(interactive: false);
    ...
    test('disableInteraction disables interaction', () {
      final ext = MinimapPlugin(interactive: true);
  ...
  group('MinimapPlugin - Position', () {
    test('setPosition updates position', () {
      final ext = MinimapPlugin(position: MinimapPosition.bottomRight);
  ...
  group('MinimapPlugin - Highlighting', () {
    ...
    test('setAutoHighlightSelection updates setting', () {
      final ext = MinimapPlugin(autoHighlightSelection: true);
  ...
  group('StatsPlugin - Construction', () {
    test('creates stats extension', () {
      final ext = StatsPlugin();

      expect(ext.id, equals('stats'));
    });
  });

  group('StatsPlugin - Node Statistics', () {
  ...
  group('StatsPlugin - Connection Statistics', () {
  ...
  group('StatsPlugin - Selection Statistics', () {
  ...
  group('StatsPlugin - Viewport Statistics', () {
  ...
  group('StatsPlugin - Summary Helpers', () {
  ...
  // ===========================================================================
  // Plugins with Controller Integration
  // ===========================================================================

  group('Plugins - Controller Integration', () {
    test('all default extensions can be resolved from config', () {
      final controller = NodeFlowController<String, dynamic>();

      // Plugins are lazily attached when accessed
      ...
      expect(controller.hasPlugin('auto-pan'), isTrue);
      expect(controller.hasPlugin('debug'), isTrue);
      expect(controller.hasPlugin('lod'), isTrue);
      expect(controller.hasPlugin('minimap'), isTrue);
      expect(controller.hasPlugin('stats'), isTrue);
    ...
    test('extensions can be accessed via controller getters', () {
      ...
      expect(controller.autoPan, isA<AutoPanPlugin>());
      expect(controller.debug, isA<DebugPlugin>());
      expect(controller.lod, isA<LodPlugin>());
    ...
    test('extensions can be added dynamically', () {
      ...
      controller.addPlugin(StatsPlugin());
    ...
    test('resolvePlugin returns registered extension', () {
      final controller = NodeFlowController<String, dynamic>();

      final autoPan = controller.resolvePlugin<AutoPanPlugin>();
    ...
    test('resolvePlugin returns null for unregistered extension', () {
      final controller = NodeFlowController<String, dynamic>(
        config: NodeFlowConfig(plugins: []),
      );
  ...
  group('Plugins - Custom Configuration', () {
    test('extensions can be customized via config', () {
      final controller = NodeFlowController<String, dynamic>(
        config: NodeFlowConfig(
          plugins: [
            AutoPanPlugin(panAmount: 25.0),
            DebugPlugin(mode: DebugMode.spatialIndex),
            LodPlugin(enabled: true, minThreshold: 0.15),
          ],
        ),
      );
