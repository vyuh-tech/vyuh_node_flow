...
The Debug plugin provides visual overlays for understanding the editor's internal state. Use it during development to
visualize the spatial index grid, autopan zones, and hit testing areas.
...
Debug is included by default but disabled. Enable it via the plugin or controller:
...
- **Zone widths**: Match `edgePadding` values from AutoPanPlugin
...
DebugPlugin(theme: DebugTheme.light)

// Dark theme - works on dark backgrounds
DebugPlugin(theme: DebugTheme.dark)
```
...
Debug mode works alongside other plugins:
...
- [AutoPan](/docs/plugins/autopan) - Configure autopan edge zones
