...
<MediaPlaceholder
  type="animation"
  title="Viewport Animation Methods"
  description="Side-by-side comparison showing animateToNode() (smooth pan+zoom to a specific node), animateToBounds() (fit selection with animation), and animateToScale() (smooth zoom to target level). Each animation uses easeInOut curve."
/>
