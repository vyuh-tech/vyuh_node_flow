...
<MediaPlaceholder
  type="video"
  title="NodeFlowEditor Overview"
  description="Quick video tour showing the editor in action: creating nodes, connecting ports, panning/zooming the canvas, selecting nodes, and using the minimap"
/>
...
<MediaPlaceholder
  type="image"
  title="Node Shapes Comparison"
  description="Side-by-side comparison showing different node shapes: rectangular (default), circle, diamond, and hexagon nodes with ports"
  aspectRatio="21/9"
/>
...
Comprehensive event handling for all editor interactions. See [Event System](/docs/node-flow/advanced/events) for complete documentation.

<Tabs items={['Node Events', 'Connection Events', 'Viewport Events']}>
  <Tab value="Node Events">
    ```dart
...
<MediaPlaceholder
  type="animation"
  title="Behavior Modes Comparison"
  description="Animated GIF showing the same graph in three modes: Design mode (full editing with connection creation), Preview mode (panning and rearranging only), and Present mode (static display with no interaction)"
/>
...
See [Keyboard Shortcuts](/docs/node-flow/advanced/keyboard-shortcuts) for the complete list and customization options.
...
- [NodeFlowViewer](/docs/node-flow/components/node-flow-viewer) - Read-only view
- [NodeFlowMinimap](/docs/node-flow/components/minimap) - Overview minimap
- [Configuration](/docs/node-flow/core-concepts/configuration) - NodeFlowConfig and AutoPanConfig
- [Theming](/docs/node-flow/theming/overview) - Customization guide
