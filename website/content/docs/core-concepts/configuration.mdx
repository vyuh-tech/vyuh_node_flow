...
<MediaPlaceholder
  type="animation"
  title="Snap-to-Grid Behavior"
  description="Split-screen animation: left side shows free-form node dragging (smooth movement), right side shows snap-to-grid enabled (nodes jump to grid intersections). Visual grid overlay shows the 20px grid cells."
  aspectRatio="21/9"
/>
...
<MediaPlaceholder
  type="animation"
  title="Autopan in Action"
  description="Animated demo showing a node being dragged toward the viewport edge, triggering smooth automatic panning that reveals more canvas space. Highlights the edge zones and shows the panning speed increasing with proximity."
/>
...
<Tabs items={['Normal', 'Fast', 'Precise']}>
  <Tab value="Normal">
    ```dart
...
<MediaPlaceholder
  type="image"
  title="Debug Mode Visualization"
  description="Screenshot showing debug overlays: semi-transparent grid cells for spatial indexing, colored edge zones for autopan regions, and bounding boxes around nodes and ports for hit testing areas"
/>
...
- [NodeFlowEditor](/docs/node-flow/components/node-flow-editor) - Main editor widget
- [Minimap](/docs/node-flow/components/minimap) - Minimap component
- [Theming](/docs/node-flow/theming/overview) - Visual customization
