...
<MediaPlaceholder
  ...
  description="Four built-in node shapes displayed side by side: Rectangle (default process node), Circle (terminal start/end), Diamond (decision/conditional), and Hexagon (preparation/setup). Each labeled with name, typical use case, and port positions."
  aspectRatio="21/9"
/>
...
<MediaPlaceholder
  type="image"
  title="Rectangle Node Shape"
  description="Standard rectangular node with ports on all four sides (left, right, top, bottom). Shows typical process step styling with content area and connection points."
/>
...
<MediaPlaceholder
  type="image"
  title="Circle Node Shape"
  description="Circular terminal node with ports at cardinal points (top, right, bottom, left). Shows green fill with darker stroke, commonly used for Start and End nodes in flowcharts."
/>
...
<MediaPlaceholder
  type="image"
  title="Diamond Node Shape"
  description="Diamond-shaped decision node with ports at the four vertices. Orange fill with deep orange stroke, showing 'Yes/No' branching pattern typical for conditional logic."
/>
...
<MediaPlaceholder
  type="image"
  title="Hexagon Node Orientations"
  description="Two hexagon orientations: Horizontal (flat top/bottom, pointed left/right) and Vertical (pointed top/bottom, flat left/right). Purple fill showing sideRatio effect on angle."
/>
...
<MediaPlaceholder
  type="image"
  title="Complete Flowchart with Mixed Shapes"
  description="Flowchart showing all shapes in action: Circle (Start) → Rectangle (Process Data) → Diamond (Valid?) with Yes branch to Circle (End) and No branch looping back. Demonstrates nodeShapeBuilder assigning shapes based on node type."
  aspectRatio="16/9"
/>
...
- [Nodes](/docs/node-flow/core-concepts/nodes) - Node concepts and properties
- [Port Shapes](/docs/node-flow/theming/port-shapes) - Customize port appearance
