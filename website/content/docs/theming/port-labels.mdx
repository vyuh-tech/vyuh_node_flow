...
<MediaPlaceholder
  type="image"
  title="Port Labels Overview"
  description="Node with labeled ports on all four sides: Left port with 'Input' label on the right, Right port with 'Output' label on the left, Top port with 'Signal' label below, Bottom port with 'Event' label above. Shows how labels automatically position inside the node boundary."
  aspectRatio="16/9"
/>
...
- [Port Shapes](/docs/node-flow/theming/port-shapes) - Customizing port appearance
