...
<MediaPlaceholder
  ...
  description="Visual keyboard showing common shortcuts: Cmd+A (Select All), Delete (Remove), F (Fit View), Cmd+D (Duplicate), arrow keys with Cmd+Shift (Alignment). Each shortcut labeled with its action. Shows platform-aware display (Cmd on macOS, Ctrl on Windows)."
  aspectRatio="21/9"
/>
...
<Callout type="info">
  **For Developers**: To customize shortcuts or create custom actions, see the [Shortcuts & Actions API](/docs/node-flow/api/shortcuts-actions) reference.
</Callout>
...
- [Shortcuts & Actions API](/docs/node-flow/api/shortcuts-actions) - Customize and extend shortcuts
- [NodeFlowEditor](/docs/node-flow/components/node-flow-editor) - Main editor component
- [Examples](/docs/node-flow/examples) - See shortcuts in action
